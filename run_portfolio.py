...
TODAY       = datetime.now()
# MONTH_END = strictly today (the script runs on the actual day, so data is only up to today)
MONTH_END   = TODAY.replace(hour=0, minute=0, second=0, microsecond=0)
    ...
    for wd in active_weeks:
        ...
        for llm_name, portfolio in wd['portfolios'].items():
            ...
            for h in portfolio:
                ...
                wt   = h['weight']
                alloc = (wt/100)*cap
                sp   = stock_data.get(sym, {}).get('prices', [100.0]*len(all_trading_days))[week_idx[0]]
...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .sp-pill{text-align:center;padding:18px 0 8px;}
  .sp-pill span{display:inline-flex;align-items:center;gap:8px;background:#f1f5f9;border:1px solid #e2e8f0;border-radius:999px;padding:9px 20px;color:#475569;font-size:clamp(.78em,2vw,.92em);font-weight:600;}
  ...
  @media(max-width:600px){
    ...
    .sp-pill span{font-size:.75em;padding:8px 14px;flex-wrap:wrap;justify-content:center;border-radius:12px;}
